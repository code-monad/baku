...
nlohmann::json baku::to_detail(const baku::meta& info) {
  auto result = nlohmann::json{{"id", info.short_id},
                               {"hash", info.sha},
...
void baku::from_detail(const nlohmann::json& json, baku::meta& info) {
  ...
  if (json.contains("chunk_hash")) json.at("chunk_hash").get_to(info.chunk_sha);
...
baku::metainfo::metainfo(const std::filesystem::path& source) {
  if (source.empty()) {
    throw std::logic_error(
        "Path is empty. Use default initialization and metainfo::parse_raw if "
        "you do not want read from file");
  }
  std::string raw;
  std::ifstream reader(source, reader.in | reader.binary);
  if (reader.is_open()) {
    raw = std::string((std::istreambuf_iterator<char>(reader)),
...
std::map<std::string, baku::meta>::iterator baku::metainfo::contains_by_sha(
    const std::string& sha) {
  for (auto iter = _detail.begin(); iter != _detail.end(); iter++) {
    auto& meta = iter->second;
    if (sha == meta.sha) {
      if (!meta.chunk_sha.empty() && meta.sha != meta.chunk_sha) {
        fmt::print(
            "warn: sha {} appears as an original data. check it by yourself!",
            sha);
      } else {
        return iter;
      }
...
void baku::metainfo::dump(const std::filesystem::path& destination,
