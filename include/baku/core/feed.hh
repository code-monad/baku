...
namespace baku {
constexpr auto generate_fn(const std::vector<std::uint8_t>& buffer,
                           unsigned width) {
  return [&buffer, width](sln::PixelIndex x, sln::PixelIndex y) {
    ...
    for (auto iter = start_iter;
         iter != buffer.end() && iter != std::next(start_iter + 3); iter++) {
...
const std::vector<std::uint8_t> feed(const std::vector<std::uint8_t>& buffer,
