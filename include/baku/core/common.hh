...
inline std::size_t pos_to_index(std::size_t x, std::size_t y,
                                std::size_t width) {
  return width * y + x;
}

inline pos_t index_to_pos(std::size_t index, std::size_t width) {
  return pos_t{index % width, index / width};
}
}  // namespace baku
